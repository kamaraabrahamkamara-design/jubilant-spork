...
def make_hash(token: str) -> str:
    """Generates a secure SHA-256 hash of the student ID string token."""
...
with tab1:
    ...
    with col1:
        student_id_input = st.text_input(
            "Student ID Token", 
            type="password", 
            value=st.session_state.student_id if not st.session_state.student_authenticated else "••••••••", 
            placeholder="Enter your secret token string"
        ).strip()
    with col2:
        ...
        if st.button("🔑 Access Portal Dashboard", use_container_width=True):
            if not student_id_input:
                ...
                hashed_input = make_hash(student_id_input)
                
                if student_id_input in valid_ids:
                    st.session_state.student_authenticated = True
                    st.session_state.student_id = student_id_input
                    st.success("✅ Portal Access Authenticated Successfully!")
                elif hashed_input in valid_ids:
                    st.session_state.student_authenticated = True
                    st.session_state.student_id = hashed_input
                    st.success("✅ Portal Access Authenticated Successfully!")
                else:
    ...
    if st.session_state.student_authenticated:
        ...
        st.info("🔒 Secure Active Session: All identifier strings have been masked.")
